/// BTCVault Sentinel — Pure WBTC Lending on Vesu PRIME
///
/// Simplest vault: deposit WBTC → Vesu PRIME pool → earn supply APY + BTCFi rewards.
/// No debt, no swap, no leverage. Lowest risk.

#[starknet::contract]
pub mod BTCVaultSentinel {
    ...
    fn constructor(
        ...
        self.name.write("BTCVault Sentinel");
        self.symbol.write("yvBTC-SENT");
    ...
    #[abi(embed_v0)]
    impl CuratorImpl of super::ISentinelCurator<ContractState> {
    ...
    #[abi(embed_v0)]
    impl VaultViewImpl of super::ISentinelView<ContractState> {
    ...
    impl InternalImpl of InternalTrait {
        ...
        fn _refresh_total_assets(ref self: ContractState) {
            ...
            let pool_addr: ContractAddress = self.vesu_pool_id.read().try_into().unwrap();
            let vesu = IVesuPoolDispatcher { contract_address: pool_addr };
            let (_position, collateral_value, _debt_value) = vesu.position(wbtc_addr, self.debt_asset.read(), this);
            let wbtc = IERC20Dispatcher { contract_address: wbtc_addr };
        ...
        fn _ensure_idle_balance(ref self: ContractState, needed: u256) {
            ...
            // Read actual on-chain position
            let (_pos, collateral_val, _) = vesu.position(wbtc_addr, self.debt_asset.read(), this);
...
#[starknet::interface]
pub trait ISentinelCurator<TContractState> {
...
#[starknet::interface]
pub trait ISentinelView<TContractState> {
